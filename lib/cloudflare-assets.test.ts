...
describe("cloudflare-assets", () => {
  ...
  it("uses the Cloudflare img hostname when edge delivery is enabled", () => {
    vi.stubEnv("NEXT_PUBLIC_R2_ENABLED", "false");
    vi.stubEnv("NEXT_PUBLIC_CF_PAGES_IMAGES_ENABLED", "false");
    vi.stubEnv("NEXT_PUBLIC_CF_EDGE_IMAGES_ENABLED", "true");
    expect(getEdgeImageUrl("/images/hero/a.webp")).toBe(
      "https://img.midtownvegascondos.com/images/hero/a.webp",
    );
    expect(getCdnImageSrc("/images/hero/a.webp")).toBe(
      "https://img.midtownvegascondos.com/images/hero/a.webp",
    );
