...
import { FollowUpBossClient, fubClient } from "./client";
...
describe("FUB Client", () => {
  ...
  it("handles rate limiting with retry", async () => {
    ...
    const retryClient = new FollowUpBossClient({
      apiKey: "test-key",
      retryAttempts: 1,
      enableRateLimiting: false,
    });

    const result = await retryClient.createLead({
      firstName: "John",
      lastName: "Doe",
      email: "john@example.com",
    });
  ...
  it("caches GET requests", async () => {
    const cachingClient = new FollowUpBossClient({
      apiKey: "test-key",
      enableCaching: true,
      enableRateLimiting: false,
      retryAttempts: 0,
    ...
    // First call
    await cachingClient.searchPeople({ email: "cached@example.com" });
