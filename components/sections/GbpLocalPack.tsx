 ...
 */
export default function GbpLocalPack({ showMap = true, className = "" }: GbpLocalPackProps) {
  return (
    <section
    ...
    >
      <div className="container mx-auto px-4 py-10 md:py-12">
        <div className={`grid gap-8 ${showMap ? "lg:grid-cols-2" : ""} items-start`}>
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-blue-400">
              Google Business Profile
            </p>
            <h2 id="gbp-local-pack-heading" className="mb-3 text-2xl font-bold md:text-3xl">
              Visit {officeInfo.name}
            </h2>
            <p className="mb-6 max-w-xl text-slate-300">
              Same name, address, and phone as Google Maps. Call, get directions, or read reviews
              before you tour Arts District and downtown Las Vegas condos.
            </p>
            ...
            <ul className="mb-6 space-y-2 text-sm text-slate-300">
              <li className="flex items-start gap-2">
                <Clock className="mt-0.5 h-4 w-4 flex-shrink-0 text-blue-400" aria-hidden />
                <span>
              ...
              <li className="flex items-center gap-2">
                <Phone className="h-4 w-4 flex-shrink-0 text-blue-400" aria-hidden />
                <a href={agentInfo.phoneTel} className="font-semibold text-white hover:text-blue-300">
                  {agentInfo.phone}
                </a>
              </li>
            </ul>
